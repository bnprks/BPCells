...
MergeFragments2::ChunkedLoader::ChunkedLoader(
...
uint32_t
MergeFragments2::ChunkedLoader::load_chunk(uint32_t *start, uint32_t *end, uint32_t *cell) {
    uint32_t copied = 0;
...
uint32_t MergeFragments2::ChunkedLoader::peek_start() {
    if (loaded == 0) {
...
void MergeFragments2::ChunkedLoader::seek(uint32_t chr_id, uint32_t base) {
    loaded = 0;
    frags->seek(chr_id, base);
}

void MergeFragments2::ChunkedLoader::restart() {
    loaded = 0;
    frags->restart();
}

int MergeFragments2::ChunkedLoader::chrCount() const { return frags->chrCount(); }
int MergeFragments2::ChunkedLoader::cellCount() const { return frags->cellCount(); }

const char *MergeFragments2::ChunkedLoader::chrNames(uint32_t chr_id) {
    return frags->chrNames(chr_id);
}
const char *MergeFragments2::ChunkedLoader::cellNames(uint32_t cell_id) {
    return frags->cellNames(cell_id);
}

bool MergeFragments2::ChunkedLoader::nextChr() {
    loaded = 0;
    return frags->nextChr();
}

uint32_t MergeFragments2::ChunkedLoader::currentChr() const { return frags->currentChr(); }

MergeFragments2::MergeFragments2(
    ...
    : chr_order(chr_order)
    , start(3 * chunk_size * fragments.size())
    , end(3 * chunk_size * fragments.size())
    , cell(3 * chunk_size * fragments.size())
    , start_buf(3 * chunk_size * fragments.size())
    , end_buf(3 * chunk_size * fragments.size())
    , cell_buf(3 * chunk_size * fragments.size())
    , heap(fragments.size())
    , load_size(load_size)
    , chunk_size(chunk_size) {
    ...
    for (uint32_t i = 0; i < fragments.size(); i++) {
        if (fragments[i]->cellCount() == -1 || fragments[i]->chrCount() == -1 ||
            ...
            throw std::runtime_error(
                "MergeFragments2 Error: all input fragments to merge must have known cell + chr "
    ...
    for (uint32_t i = 0; i < fragments.size(); i++) {
        ...
        for (int32_t j = 0; j < chr_count; j++) {
            if (chr_order_lookup.find(f->chrNames(j)) != chr_order_lookup.end()) {
                ...
                throw std::runtime_error(
                    "MergeFragments2 Error: Input index " + std::to_string(i) + " has chromosome " +
...
bool MergeFragments2::isSeekable() const { return true; }

void MergeFragments2::seek(uint32_t chr_id, uint32_t base) {
...
int MergeFragments2::chrCount() const {
    return chr_order.size();
}

int MergeFragments2::cellCount() const {
    return cell_id_offset.back();
}

const char *MergeFragments2::chrNames(uint32_t chr_id) {
    if (chr_id >= chr_order.size()) return NULL;
    return chr_order[chr_id].c_str();
}

const char *MergeFragments2::cellNames(uint32_t cell_id) {
...
uint32_t MergeFragments2::currentChr() const { return current_chr; }


bool MergeFragments2::load() {
    ...
    if (cur_idx >= available) {
        ...
        for (uint32_t i = 0; i < 2*frags.size(); i++) {
            ...
            loaded += frags[idx].load_chunk(start.data() + loaded, end.data() + loaded, cell.data() + loaded);
...
uint32_t MergeFragments2::capacity() const { return std::min(available - cur_idx, load_size); }

uint32_t *MergeFragments2::cellData() { return cell.data() + cur_idx; }
uint32_t *MergeFragments2::startData() { return start.data() + cur_idx; }
uint32_t *MergeFragments2::endData() { return end.data() + cur_idx; }
} // namespace BPCells