...
void MergeFragments::seek(uint32_t chr_id, uint32_t base) {
    for (auto f: frags) f.seek(chr_id, base);
    heap.clear();
}
...
void MergeFragments::restart() {
    ...
    for (auto &c: frags_completed) c = 0;
}
...
int MergeFragments::chrCount() const {
    ...
    for (auto f: frags) {
        ...
        if (f.chrCount() != count) throw std::runtime_error("Not all merged fragments have the same chrCount. Select identical chromosomes before merging.");
    }
    return count;
}
    ...
    for (auto f: frags) {
        ...
        if (strcmp(name, f_name) != 0) {
            throw std::runtime_error(
                ...
                std::string(" mismatch: ") + std::string(f_name) + 
                std::string(" vs. ") + std::string(name)
            );
        }
    }
    return name;
...
bool MergeFragments::nextChr() {
    ...
    bool any_ready = false; // Check that we have some fragments ready to read from the current chromosome
    while (!any_ready) {
        current_chr += 1;
        ...
        for (uint32_t i = 0; i < frags.size(); i++) {
            ...
            if (prev_chr >= frags[i].currentChr())
                    throw std::runtime_error("Fragments to merge have out-of-order chromosomes: " + std::to_string(i));
            if (frags[i].currentChr() == current_chr)
                any_ready = true;
        }
        if (!any_remaining) {
    ...
    for (uint32_t i = 0; i < frags.size(); i++) {
        ...
        if (f_name == NULL || chr_name == NULL || strcmp(chr_name, f_name) != 0) {
            throw std::runtime_error(
                std::string("MergeFragments: Names for chromosome ID ") + std::to_string(current_chr) +
                std::string(" mismatch: ") + std::string(f_name) + 
...
bool MergeFragments::load() {
    if (heap.empty()) {
        ...
        dary_heap::make_heap<4>(heap.begin(), heap.end(), std::bind(&MergeFragments::compare, this, std::placeholders::_1, std::placeholders::_2));
    }
    uint32_t i;
    for (i = 0; i < start.size() && !heap.empty(); i++) {
        ...
        dary_heap::pop_heap<4>(heap.begin(), heap.end(), std::bind(&MergeFragments::compare, this, std::placeholders::_1, std::placeholders::_2));
        if (frags[idx].nextFrag()) {
            // Just add fragment source back into the heap
            dary_heap::push_heap<4>(heap.begin(), heap.end(), std::bind(&MergeFragments::compare, this, std::placeholders::_1, std::placeholders::_2));
        } else {
            heap.pop_back();
        }
    }
