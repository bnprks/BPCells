...
template<typename T>
class SparseMultiply : public MatrixLoader<T> {
protected:
    // Running invariants: 
...
public:
    SparseMultiply(MatrixLoader<T> &left, MatrixLoader<T> &right, uint32_t load_size=1024) :
        left(left), right(right), col_buffer(left.rows()), val_buffer(load_size), row_buffer(load_size), max_load_size(load_size) {
    ...
    bool load() override {
        if (row_idx == 0) {
            ...
            for (auto &x : col_buffer) {x = 0;}
            // For each entry in the current column of right, add entry_val * left[:,entry_row] to the col_buffer
            while (right.load()) {
