...
template<typename T>
class ConcatRows : public MatrixLoader<T> {
protected:
    std::vector<MatrixLoader<T>*> mats;
    std::vector<uint32_t> row_offset;
    uint32_t cur_mat = 0;
public:
    ConcatRows(const std::vector<MatrixLoader<T>*> &mats) :
        mats(mats) {

        if (mats.size() < 2)
            throw std::runtime_error("Must have >= 2 matrices to merge");
    ...
    bool nextCol() override {
        ...
        if (any_fail) throw std::runtime_error("ConcatRows: Some matrices reached nextCol while others did not");
        return true;
    }
...
template<typename T>
class ConcatCols : public MatrixLoader<T> {
protected:
    std::vector<MatrixLoader<T> *> mats;
    std::vector<uint32_t> col_offset;
    uint32_t cur_mat = 0;
public:
    ConcatCols(const std::vector<MatrixLoader<T> *> &mats) :
        mats(mats) {
    ...
    void seekCol(uint32_t col) override {
        ...
        if (idx == mats.size()) throw std::runtime_error("ConcatCols: Cannot seek to a column larger than number of columns");
    ...
    // Return the index of the current column
    uint32_t currentCol() const override {return mats[cur_mat]->currentCol() + col_offset[cur_mat];}
