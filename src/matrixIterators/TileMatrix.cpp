        ...
        while (i < capacity) {
            ...
            if (items == 128) end_max = std::max(simdmax(end_data + i), end_max);
                ...
                for (uint32_t k = i; k < capacity; k++)
                    end_max = std::max(end_max, end_data[k]);
            }
            ...
            for (uint32_t j = 0; j < active_tiles.size(); j++) {
                ...
                for (k = 0; k + 4 <= items && start_data[i + k] < t.end; k += 4) {
                    ...
                    BPCells::store(
                        (vec *)&has_overlap[0][k],
                        bitwise_and(
                            bitwise_xor(bit_flip, cmp_lt_signed(start, t_start)),
                            cmp_lt_signed(start, t_end)
                        )
                    );
                    // tile[0][k] = t.output_idx + (start_data[k] - t.start) / t.width
                    BPCells::store(
                        (vec *)&tile[0][k], add(t_idx, libdivide_vec(sub(start, t_start), &t.width))
                    );
                    ...
                    BPCells::store(
                        (vec *)&has_overlap[1][k],
                        bitwise_and(
                            cmp_gt_signed(end, t_start),
                            bitwise_xor(bit_flip, cmp_gt_signed(end, t_end))
                        )
                    );
                    // tile[0][k] = t.output_idx + (end_data[k] - 1 - t.start) / t.width
                    BPCells::store(
                        (vec *)&tile[1][k],
                        add(t_idx, libdivide_vec(sub(end, add(t_start, splat(1U))), &t.width))
                    );
                }
                for (; k < items && start_data[i + k] < t.end; k++) {
                    has_overlap[0][k] = start_data[i + k] >= t.start && start_data[i + k] < t.end;
                    tile[0][k] = t.output_idx +
                                 libdivide::libdivide_u32_do(start_data[i + k] - t.start, &t.width);

                    has_overlap[1][k] = end_data[i + k] > t.start && end_data[i + k] <= t.end;
                    tile[1][k] =
                        t.output_idx +
                        libdivide::libdivide_u32_do(end_data[i + k] - 1 - t.start, &t.width);
                }
