...
private:
    ...
    uint32_t col;
public:
    CSparseMatrix(const EigenMat mat, std::unique_ptr<StringReader> &&row_names = NULL, 
                  std::unique_ptr<StringReader> &&col_names = NULL, uint32_t load_size = 1024) : 
        mat(mat), row_names(std::move(row_names)), col_names(std::move(col_names)), load_size(load_size) {
