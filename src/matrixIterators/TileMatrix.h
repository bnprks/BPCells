...
// The last tile in a region may be truncated if the region is not an even multiple of the tile width
...
private:
    ...
    uint32_t current_output_tile = UINT32_MAX; // Index of the current column we are trying to output
...
public:
    ...
    TileMatrix(FragmentLoader &frags, 
