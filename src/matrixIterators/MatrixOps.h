...
template<typename T>
Eigen::MatrixXd MatrixLoader<T>::denseMultiplyRight(const Eigen::Map<Eigen::MatrixXd> B, void (*checkInterrupt)(void)) {
...
template<typename T>
Eigen::MatrixXd MatrixLoader<T>::denseMultiplyLeft(const Eigen::Map<Eigen::MatrixXd> B, void (*checkInterrupt)(void)) {
...
template<typename T>
Eigen::VectorXd MatrixLoader<T>::vecMultiplyRight(const Eigen::Map<Eigen::VectorXd> v, void (*checkInterrupt)(void)) {
...
template<typename T>
Eigen::VectorXd MatrixLoader<T>::vecMultiplyLeft(const Eigen::Map<Eigen::VectorXd> v, void (*checkInterrupt)(void)) {
...
template<typename T>
std::vector<T> MatrixLoader<T>::colSums(void (*checkInterrupt)(void)) {
...
template<typename T>
std::vector<T> MatrixLoader<T>::rowSums(void (*checkInterrupt)(void)) {
...
template<typename T>
StatsResult MatrixLoader<T>::computeMatrixStats(Stats row_stats, Stats col_stats, void (*checkInterrupt)(void)) {
    restart();
    StatsResult res {
        Eigen::ArrayXXd((int) row_stats, rows()),
        Eigen::ArrayXXd((int) col_stats, cols())
    };
    ...
    if (col_stats == Stats::Mean) {
        ...
        res.col_stats.row(2) = (m2 + mean.square() * nnz * ((double) n_rows - nnz) / n_rows) /
            (n_rows - 1);
        res.col_stats.row(1) = mean * nnz / n_rows;
    }
    ...
    if (row_stats == Stats::Mean) {
        ...
        res.row_stats.row(2) = (m2 + mean.square() * nnz * ((double) n_cols - nnz) / n_cols) /
            (n_cols - 1);
        res.row_stats.row(1) = mean * nnz / n_cols;
    }
