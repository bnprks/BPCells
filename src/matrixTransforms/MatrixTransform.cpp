...
MatrixTransform::MatrixTransform(MatrixLoader<double> &loader) : MatrixLoaderWrapper<double>(loader) {}
MatrixTransform::MatrixTransform(MatrixLoader<double> &loader, TransformFit fit)
    : MatrixLoaderWrapper<double>(loader), fit(fit) {}
...
bool MatrixTransformDense::load() {
    ...
    for (uint32_t i = 0; i < load_size; i++) row_data[i] = current_row + i;
    ...
    while (loader_capacity > 0) {
        if (loader_idx >= loader_capacity ) {
            if (!loadZeroSubtracted()) loader_capacity = 0;
...
uint32_t MatrixTransformDense::capacity() const {
    ...
    return current_row != loader.rows() ? buf_size : current_row - ((current_row - 1) / buf_size)*buf_size;
}
uint32_t* MatrixTransformDense::rowData() {return row_data.data();}
double* MatrixTransformDense::valData() {return val_data.data();}

Eigen::MatrixXd MatrixTransformDense::denseMultiplyRight(const Eigen::Map<Eigen::MatrixXd> B, void (*checkInterrupt)(void)) {
...
Eigen::MatrixXd MatrixTransformDense::denseMultiplyLeft(const Eigen::Map<Eigen::MatrixXd> B, void (*checkInterrupt)(void)) {
    // Perform the denseMultiplyRight operation like in MatrixOps.h, but using 
    // loadZeroSubtracted in place of load
...
Eigen::VectorXd MatrixTransformDense::vecMultiplyRight(const Eigen::Map<Eigen::VectorXd> v, void (*checkInterrupt)(void)) {
...
Eigen::VectorXd MatrixTransformDense::vecMultiplyLeft(const Eigen::Map<Eigen::VectorXd> v, void (*checkInterrupt)(void)) {
