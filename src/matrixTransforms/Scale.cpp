...
Scale::Scale(MatrixLoader<double> &mat, TransformFit fit) 
    : MatrixTransform(mat, fit) {}
...
Eigen::MatrixXd Scale::denseMultiplyRight(const Eigen::Map<Eigen::MatrixXd> B, void (*checkInterrupt)(void)) {
    Eigen::MatrixXd res;
    
    // Scale input by col scale
    if (fit.col_params.size() > 0) {
        Eigen::MatrixXd B2(B.array().colwise() * fit.col_params.row(0).transpose());
        res = loader.denseMultiplyRight(Eigen::Map<Eigen::MatrixXd>(B2.data(), B2.rows(), B2.cols()), checkInterrupt);
    } else {
...
Eigen::MatrixXd Scale::denseMultiplyLeft(const Eigen::Map<Eigen::MatrixXd> B, void (*checkInterrupt)(void)) {
    Eigen::MatrixXd res;
    
    // Scale input by row scale
    if (fit.row_params.size() > 0) {
        Eigen::MatrixXd B2(B.array().rowwise() * fit.row_params.row(0));
        res = loader.denseMultiplyLeft(Eigen::Map<Eigen::MatrixXd>(B2.data(), B2.rows(), B2.cols()), checkInterrupt);
    } else {
...
Eigen::VectorXd Scale::vecMultiplyRight(const Eigen::Map<Eigen::VectorXd> v, void (*checkInterrupt)(void)) {
    Eigen::VectorXd res;
    
    // Scale input by col scale
    if (fit.col_params.size() > 0) {
        Eigen::VectorXd v2(v.array() * fit.col_params.row(0).transpose());
        res = loader.vecMultiplyRight(Eigen::Map<Eigen::VectorXd>(v2.data(), v2.size()), checkInterrupt);
    } else {
...
Eigen::VectorXd Scale::vecMultiplyLeft(const Eigen::Map<Eigen::VectorXd> v, void (*checkInterrupt)(void)) {
    Eigen::VectorXd res;
    
    // Scale input by row scale
    if (fit.row_params.size() > 0) {
        Eigen::VectorXd v2(v.array() * fit.row_params.row(0).transpose());
        res = loader.vecMultiplyLeft(Eigen::Map<Eigen::VectorXd>(v2.data(), v2.size()), checkInterrupt);
    } else {
