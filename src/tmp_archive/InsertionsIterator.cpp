...
InsertionsIterator::InsertionsIterator(FragmentsLoader &loader, uint32_t buffer_size)
    ...
    , idx(buffer_size)
    , start_buf(buffer_size)
    , end_buf(buffer_size)
    , cell_buf(buffer_size) {

    if (buffer_size < 128) throw std::invalid_argument("buffer_size must be >= 128");
...
int32_t InsertionsIterator::load(uint32_t count, FragmentArray buffer) {
