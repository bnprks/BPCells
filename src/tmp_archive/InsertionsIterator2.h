...
void InsertionsIterator2::loadInsertions() {
    ...
    FragmentArray buf = {
        &start_buf.coord[0],
        &end_sort_scratch.coord[ends_remaining],
        &start_buf.cell[0],
        fragments_to_load};
    ...
    if (load_count == 0) {
        ...
        std::memmove(
            &end_sort_scratch.cell[0], &end_buf.cell[next_end], ends_remaining * sizeof(uint32_t)
        );
        std::memmove(
            &end_sort_scratch.coord[0], &end_buf.coord[next_end], ends_remaining * sizeof(uint32_t)
        );
        // Copy loaded cells into buffer
        std::memmove(
            &end_sort_scratch.cell[ends_remaining],
            &start_buf.cell[0],
            load_count * sizeof(uint32_t)
        );
