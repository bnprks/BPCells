...
Eigen::MatrixXd denseMultiplyRight(
    MatrixIterator<double> &A, const Eigen::Map<Eigen::MatrixXd> B, bool transpose_A = false
);
...
Eigen::VectorXd vecMultiplyRight(
    MatrixIterator<double> &A, const Eigen::Map<Eigen::VectorXd> v, bool transpose_A = false
);
...
template <typename T> std::vector<T> colSums(MatrixIterator<T> &m) {
...
template <typename T> std::vector<T> rowSums(MatrixIterator<T> &m) {
