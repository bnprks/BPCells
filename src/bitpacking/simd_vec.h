...
#else
using vec = struct { uint32_t x0, x1, x2, x3; };
#endif
...
inline vec libdivide_vec(const vec &v, const struct libdivide::libdivide_u32_t *divisor) {
    return libdivide::libdivide_u32_do_vec128(v, divisor);
}
...
inline vec shift_r(const vec &v, const uint32_t i) {
    return vshlq_u32(v, vdupq_n_s32(-((int32_t)i)));
}
// Arithmetic shift that shifts in the sign bit
inline vec shift_r_arith(const vec &v, const uint32_t i) {
    return (vec)vshlq_s32((int32x4_t)v, vdupq_n_s32(-((int32_t)i)));
}
...
inline vec cmp_lt_signed(const vec &v1, const vec &v2) {
    return vcltq_s32((int32x4_t)v1, (int32x4_t)v2);
}
inline vec cmp_gt_signed(const vec &v1, const vec &v2) {
    return vcgtq_s32((int32x4_t)v1, (int32x4_t)v2);
}
...
inline vec load(vec const *mem_addr) {
    uint32_t *m = (uint32_t *)mem_addr;
    return {m[0], m[1], m[2], m[3]};
}
inline vec load_aligned(vec const *mem_addr) { return load(mem_addr); }

inline void store(vec *mem_addr, vec v) {
    uint32_t *m = (uint32_t *)mem_addr;
    m[0] = v.x0;
    m[1] = v.x1;
    m[2] = v.x2;
...
inline vec shift_l(const vec &v, uint32_t i) {
    return {v.x0 << i, v.x1 << i, v.x2 << i, v.x3 << i};
}
inline vec shift_r(const vec &v, uint32_t i) {
    return {v.x0 >> i, v.x1 >> i, v.x2 >> i, v.x3 >> i};
}
// Arithmetic shift that shifts in the sign bit
...
inline vec bitwise_or(const vec &v1, const vec &v2) {
    return {v1.x0 | v2.x0, v1.x1 | v2.x1, v1.x2 | v2.x2, v1.x3 | v2.x3};
}
inline vec bitwise_and(const vec &v1, const vec &v2) {
    return {v1.x0 & v2.x0, v1.x1 & v2.x1, v1.x2 & v2.x2, v1.x3 & v2.x3};
}
inline vec bitwise_xor(const vec &v1, const vec &v2) {
    return {v1.x0 ^ v2.x0, v1.x1 ^ v2.x1, v1.x2 ^ v2.x2, v1.x3 ^ v2.x3};
}
...
// Input: [a,b,c,d]; [?,?,?,h]; Output [a-h, b-a, c-b, d-c]
inline vec delta(const vec &v, const vec &initOffset) {
    return {v.x0 - initOffset.x3, v.x1 - v.x0, v.x2 - v.x1, v.x3 - v.x2};
}
#endif
...
inline uint32_t maxbitas32int(const vec accumulator) {
    const vec _tmp1 = bitwise_or(
        move_l_2(accumulator), accumulator
    ); /* (A,B,C,D) or (0,0,A,B) = (A,B,C or A,D or B)*/
    const vec _tmp2 =
        bitwise_or(move_l_1(_tmp1), _tmp1); /*  (A,B,C or A,D or B) or  (0,0,0,C or A)*/
...
inline void
maxdiffbitsas32int(const vec mins, const vec maxs, uint32_t &bit_count, uint32_t &minvalue) {
