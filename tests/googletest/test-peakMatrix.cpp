...
bool matrix_identical_cpp(MatrixLoader<uint32_t> &mat1,
                          MatrixLoader<uint32_t> &mat2);
...
TEST(PeakMatrix, PeakMatrix) {
  ...
  std::unique_ptr<StringWriter> w_cell_names =
      v.createStringWriter("cell_names");
  ...
  w_cell_names->write(
      VecStringReader(std::vector<std::string>{"c0", "c1", "c2", "c3", "c4"}));
  ...
  PeakMatrix m(frags, chr, start, end,
               std::make_unique<VecStringReader>(
...
TEST(PeakMatrix, PeakMatrixSeek) {
  ...
  auto v = Testing::writeFragmentTuple(
      Testing::generateFrags(50000, chrs, max_coord, 50, 125));
  StoredFragments frags = StoredFragments::openUnpacked(*v);
  ...
  PeakMatrix peak_mat(frags, v_chr, v_start, v_end,
...
TEST(PeakMatrix, TileMatrix) {
  ...
  TileMatrix m(frags, chr, start, end, width,
               std::make_unique<VecStringReader>(
                   std::vector<std::string>{"chr1", "chr2"}));
...
TEST(PeakMatrix, TileMatrixSeek) {
  ...
  TileMatrix tile_mat(frags, v_chr, v_start, v_end, v_width,
                      std::make_unique<VecStringReader>(chr_levels));
    ...
    while (true) {
      ...
      if (i1.row() != i2.row() || i1.col() != i2.col() ||
        ...
        printf("Mismatched entries: (%d,%d=%d) vs. (%d,%d=%d)\n", i1.row(),
               i1.col(), i1.val(), i2.row(), i2.col(), i2.val());
        return false;
      }
    }
  }
