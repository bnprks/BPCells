...
MatrixXd generate_dense_mat(uint32_t n_row, uint32_t n_col,
                            uint32_t seed = 125124) {
...
template <typename T> Map<T> get_map(T &mat) {
  return Map<T>(mat.data(), mat.rows(), mat.cols());
}

template <>
Map<SparseMatrix<double>>
get_map<SparseMatrix<double>>(SparseMatrix<double> &mat) {
  return Map<SparseMatrix<double>>(
      mat.rows(), mat.cols(), mat.nonZeros(), (int *)mat.outerIndexPtr(),
      (int *)mat.innerIndexPtr(), (double *)mat.valuePtr());
}
...
TEST(MatrixMath, Stats) {
  ...
  row_stats.row(2) =
      (a1.colwise() - row_stats.row(1).transpose()).square().rowwise().sum() /
      (a1.cols() - 1);

  col_stats.row(0) = (a1 > 0).colwise().count().cast<double>();
  col_stats.row(1) = a1.colwise().mean();
  col_stats.row(2) =
      (a1.rowwise() - col_stats.row(1)).square().colwise().sum() /
      (a1.rows() - 1);
...
void checkMultiplyOps(MatrixLoader<double> &mat, MatrixXd res) {
  ...
  EXPECT_TRUE(
      (res * mat_right)
          .isApprox(mat.denseMultiplyRight(get_map<MatrixXd>(mat_right))));
  EXPECT_TRUE(
      (res * vec_right)
          .isApprox(mat.vecMultiplyRight(get_map<VectorXd>(vec_right))));

  EXPECT_TRUE(
      (mat_left * res)
          .isApprox(mat.denseMultiplyLeft(get_map<MatrixXd>(mat_left))));
...
TEST(MatrixMath, Scale) {
  ...
  // Scale row + col
  MatrixXd ans1 =
      (MatrixXd(m1).array().rowwise() * scale_col.row(0)).colwise() *
...
public:
  SimpleRowShift(MatrixLoader<double> &mat, TransformFit fit)
      : MatrixTransformDense(mat, fit) {}

  bool loadZeroSubtracted() override { return loader.load(); }
  void loadZero(double *values, uint32_t count, uint32_t start_row,
