...
void test_order_rows(SparseMatrix<double> m, uint32_t load_size) {
  std::vector<uint32_t> col(m.outerIndexPtr(),
                            m.outerIndexPtr() + m.cols() + 1);
  std::vector<uint32_t> row(m.innerIndexPtr(),
  ...
  StoredMatrix<double> unordered(
      ...
      DoubleReader(
          std::make_unique<VecNumReader<double>>(val.data(), val.size()), 16),
      UIntReader(std::make_unique<VecUIntReader>(col.data(), col.size()), 16),
      row_count, std::make_unique<VecStringReader>(std::vector<std::string>()),
  ...
  StoredMatrix<double> orig(
      UIntReader(
          std::make_unique<VecUIntReader>(row_orig.data(), row_orig.size()),
          16),
      DoubleReader(std::make_unique<VecNumReader<double>>(val_orig.data(),
                                                          val_orig.size()),
                   16),
      UIntReader(std::make_unique<VecUIntReader>(col.data(), col.size()), 16),
      row_count, std::make_unique<VecStringReader>(std::vector<std::string>()),
