...
#'  - TSSEnrichment: AvgInsertInTSS / max(AvgInsertFlankingTSS, 0.1), where AvgInsertInTSS is insertions +/- 50bp 
#'    from TSS divided by 101 (window size), and AvgInsertFlankingTSS is insertions 1901-2000bp from TSS divided by 400
#'    (window size). The max(0.1) ensures that very low-read cells do not get assigned spuriously high TSSEnrichment
...
qc_stats <- function(fragments, genes, blacklist, zero_based_coords=TRUE) {
    ...
    regions <- regions[order(GenomicRanges::seqnames(regions), GenomicRanges::end(regions), GenomicRanges::start(regions))]

    membership_mat <- matrix(0, nrow=length(regions), ncol=4)
    membership_mat[matrix(c(seq_along(regions), regions$origin), ncol=2)] <- 1
