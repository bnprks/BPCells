...
#' @export
gene_score_tiles <- function(genes, chromosome_sizes = NULL, tile_size = 500, addArchRBug = FALSE) {
  assert_is_wholenumber(tile_size)
  ...
  first_tile <- extended$start %/% tile_size
  last_tile <- (extended$end - 1) %/% tile_size
...
#'
#' Given a set of tile coordinates and distances returned by `gene_score_tiles()`,
...
#' @inheritParams gene_score_tiles
...
#' @export
gene_score_weights_archr <- function(genes, chromosome_sizes, blacklist = NULL, tile_size = 500, addArchRBug = FALSE) {
  ...
  tiles <- gene_score_tiles(genes, chromosome_sizes = chromosome_sizes, addArchRBug = addArchRBug)
  ...
  tile_size <- tiles$end[1] - tiles$start[1]
  tile_count <- chromosome_sizes %>%
    dplyr::mutate(tile_count = (end - start + tile_size - 1) %/% tile_size) %>%
    dplyr::pull(tile_count, name = chr)
  start_tile <- cumsum(tile_count) - tile_count

  res <- sparseMatrix(
    i = tiles$gene_idx,
    j = start_tile[as.character(tiles$chr)] + tiles$start %/% tile_size + 1,
