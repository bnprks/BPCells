...
#' Compute LSI For a peak matrix 
#' @param mat PeakMatrix
#' @param n_dimensions Number of dimensions to keep during PCA
#' @param scale_factor Scale factor for the tf-idf log transform
#' @param verbose Whether to print out progress
#' @param threads Number of threads to use
#' @return dgCMatrix of shape (n_dimensions, ncol(mat))
#' @details Compute LSI through first doing a tf-idf transform, a z-score normalization, then PCA.  
#' Tf-idf implementation is from Stuart & Butler et al. 2019.  
#' @export
compute_lsi <- function(mat, n_dimensions = 50, scale_factor = 1e-4, verbose = FALSE, threads = 1) {
  assert_is(mat, "IterableMatrix") # Should be a peak matrix, should we enforce this?
  assert_is(n_dimensions, "integer")
  ...
  # run z-score norm
  cell_peak_stats <- matrix_stats(mat_log_tfidf, col_stats="variance")$col_stats
  ...
  # Run pca
  svd_attr_ <- svds(mat_lsi_norm, k = n_dimensions)
  pca_res <- t(svd_attr$u) %*% mat_lsi_norm
  return(pca_res)
}

#' Get most variable features, given a non-log normalized matrix
highly_variable_features <- function(mat, num_feats, n_bins, verbose = FALSE) {
  assert_is(mat, "IterableMatrix")
  assert_is(num_feats, "integer")
  assert_greater_than_zero(num_feats)
  assert_len(num_feats, 1)
  assert_is(n_bins, "integer")
  assert_len(n_bins, 1)
  assert_greater_than_zero(n_bins)
  if (nrow(mat) <= num_feats) {
    if (verbose) log_progress(sprintf("Number of features (%s) is less than num_feats (%s), 
                                       returning all features", nrow(mat), num_feats))
    return(mat)
  }
  # Calculate the mean and variance of each feature
  # should we set entries that are 0 to NA?
  ...
  # Calculate the mean and variance of dispersion of each bin
  bin_mean <- tapply(feature_dispersion, mean_bins, function(x) mean(x))
  bin_sd <- tapply(feature_dispersion, mean_bins, function(x) sd(x))
  # Set bin_sd value to bin_mean if bin_sd is NA, results in norm dispersion of 1
  bin_sd[is.na(bin_sd)] <- bin_mean[is.na(bin_sd)]
  # map mean_bins indices to bin_stats
  feature_dispersion_norm <- (feature_dispersion - bin_mean[mean_bins]) / bin_sd[mean_bins]
  names(feature_dispersion_norm) <- names(feature_dispersion)
  variable_features_ <- sort(feature_dispersion_norm)[nrow(mat)-num_feats:nrow(mat)]
