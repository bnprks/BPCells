...
PseudobulkStats pseudobulk_matrix(std::unique_ptr<MatrixLoader<T>> &&mat,
    ...
    if (method & PseudobulkStatsMethod::NonZeros) res.non_zeros = Eigen::ArrayXXd::Zero(num_rows, num_cols);
    if (method & PseudobulkStatsMethod::Sum) res.sum = Eigen::ArrayXXd::Zero(num_rows, num_cols);
    if (method & PseudobulkStatsMethod::Mean) res.mean = Eigen::ArrayXXd::Zero(num_rows, num_cols);
    ...
    while (it.nextCol()) {
        ...
        while (it.load()) {
            ...
            if (transpose) {
                ...
                } else if (method & PseudobulkStatsMethod::Mean) {
                ...
                } else if (method & PseudobulkStatsMethod::NonZeros) {
                    for (uint32_t i = 0; i < count; i++) res.non_zeros(cell_groups[row_data[i]], col)++;
                }
                if (method & PseudobulkStatsMethod::Sum) {
    ...
    if (transpose) {
        ...
        if (method & PseudobulkStatsMethod::NonZeros) res.non_zeros.transposeInPlace();
        if (method & PseudobulkStatsMethod::Sum) res.sum.transposeInPlace();
        if (method & PseudobulkStatsMethod::Mean) res.mean.transposeInPlace();
