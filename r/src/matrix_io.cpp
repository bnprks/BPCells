...
///////// EXPERIMENTAL FILE MATRIX FUNCTIONS //////////

// [[Rcpp::export]]
SEXP EXPERIMENTAL_iterate_packed_sparse_column_matrix_file_uint32_t_cpp(
    std::string dir,
    uint32_t buffer_size
) {
    FileReaderBuilder rb(dir, buffer_size);
    return make_unique_xptr<StoredMatrix<uint32_t>>(EXPERIMENTAL_openPackedSparseColumn<uint32_t>(
        rb,
        1024
    ));
}
...
// [[Rcpp::export]]
void EXPERIMENTAL_write_packed_sparse_column_matrix_file_uint32_t_cpp(
    SEXP matrix, std::string dir, uint32_t buffer_size, bool allow_overwrite, bool row_major
) {
    FileWriterBuilder wb(dir, buffer_size, allow_overwrite);
    ...
    run_with_R_interrupt_check(
        ...
        EXPERIMENTAL_createPackedSparseColumn<uint32_t>(wb, row_major),
        std::ref(*loader)
    );
}
...
List EXPERIMENTAL_dims_packed_sparse_column_matrix_file_cpp(std::string dir, uint32_t buffer_size) {
    ...
    List l = dims_matrix(EXPERIMENTAL_openPackedSparseColumn<uint32_t>(rb), row_major);
    l["compressed"] = true;
    l["type"] = "uint32_t";
    return l;
}
...
void write_matrix_anndata_hdf5_dense_base(
    ...
    H5DenseMatrixWriter<T> w = createAnnDataDenseMatrix<T>(file, dataset, loader->rows(), loader->cols(), row_major, buffer_size, chunk_size, gzip_level);
